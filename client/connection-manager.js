class ConnectionManager{
	...
	updateManager(peers){
		const me = peers.you;
		// const clients = peers.clients.filter(id => me !== id); //previous
		// const clients = peers.clients.filter(client.id => me !== client.id); //his demo, argument doesn't make sense
		const clients = peers.clients.filter(client => me !== client.id); //my adjustment
	...
	updatePeer(id, fragment, [prop, value]){
		if(!this.peers.has(id)){
			console.error(`Client doesn't exist, `, id);
			return;
		}
